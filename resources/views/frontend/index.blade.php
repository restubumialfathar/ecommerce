 ...
 <section id="heroSection" class="container relative max-w-screen-xl pt-10 pb-24">
    <div class="flex flex-wrap items-center justify-between w-full gap-8">
        <div class="w-full max-w-[400px] flex flex-col gap-4">
            <div class="inline-flex gap-[6px] items-center bg-hk rounded-full px-4 py-[6px] w-max">
            ...
            <h1 class="text-[36px] md:text-[48px] text-white font-bold">
                Empower Your
                <span
                    class="text-dark-indigo bg-mr inline-flex items-center h-[49px] w-max">Passions</span>
                Today
            </h1>
            ...
            <div class="mt-[14px]">
                <a href="#eventSection" class="btn-secondary text-white">
                    Explore Now
                </a>
            </div>
        </div>
 ...
 <section id="eventSection" class="container relative max-w-screen-xl py-10">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-[30px]">
        ...
        <x-frontend.card-event :cover="$event->thumbnail" :title="$event->name" :category="$event->category->name" :date="$event->start_time" :price="$event->start_from"
                               :isPopular="$event->is_popular" :description="$event->headline" :routes="route('detail', $event->slug)" />
      @endforeach
    </div>
</section>
...
<section id="categoriesSection" class="relative pb-[100px] overflow-hidden">
    <div class="container relative max-w-screen-xl py-10">
        ...
        <div class="flex justify-between items-center gap-4 mb-[50px]">
            ...
            @if (!request()->has('all_category'))
            <a href="{{ request()->fullUrlWithQuery(['all_category' => 1]) }}" class="btn-primary">
              View All
            </a>
          @endif
        </div>
